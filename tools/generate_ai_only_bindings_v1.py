...
for rid, name, structural_level, lineage in src.get('records', []):
    ...
    bindings.append({
        ...
        'binding_status': 'SOURCE_LINEAGE_DERIVED_CROSSWALK' if caps and not unknown_caps else 'GAP_REVIEW_REQUIRED',
        'unknown_source_cap_ids': unknown_caps,
        'authority_note': 'AI-NEW identity/name/level are approved source records. AI/ASI segments, candidate containers, Sequence roles and ASI Nodes are additive Phase-2 mappings derived through source AI-CAP lineage; they do not rewrite the source record.'
    })
...
payload={
    ...
    'mapping_sources':['registries/ai/AI_CONTAINER_CANDIDATES_V0.json','registries/asi/ASI_CONTAINER_CANDIDATES_FROM_AI_SOURCES_V0.json','generated/registry_views/ai_native_candidate_registry_v0.json'],
    'record_count':64,
    'rule':'Approved AI-NEW records stay native. Phase-2 AI/ASI/Sequence/Node bindings are derived through their source AI-CAP lineage and remain separately auditable.',
    'records':bindings
}
(OUT/'ai_only_64_phase2_bindings_v1.json').write_text(json.dumps(payload,indent=2),encoding='utf-8')
print('Generated AI-NEW bindings:',len(bindings),'gaps:',sum(b['binding_status']!='SOURCE_LINEAGE_DERIVED_CROSSWALK' for b in bindings))
