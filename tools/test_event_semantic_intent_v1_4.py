...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for case in fx.get("cases",[]):
            ...
            d=load(out); ledger=d.get("event_growth_ledger") or {}; counts=ledger.get("counts") or {}; intents=d.get("intent_hypotheses",[]); sem=d.get("semantic_intent_rule_hits",[]); checks={"v1_4":d.get("interpreter_version")=="P2-MICRO-SEQUENCE-LIVE-INTERPRETER-001/V1.4","events_have_intent":bool(d.get("event_records")) and all(e.get("intent",{}).get("intent_type") for e in d.get("event_records",[])),"semantic_or_existing_activation_route":bool(sem) or bool(d.get("intent_signals")),"live_intent_generated":len(intents)>0,"canonical_parameter_zero":counts.get("canonical_parameter_additions")==0,"canonical_pattern_zero":counts.get("canonical_pattern_additions")==0,"validated_support_zero":counts.get("validated_support_delta")==0,"bounded_live_intents":len(intents)<=12}
            for n,ok in checks.items():
                if not ok: errors.append(f"{case['case_id']}: failed {n}")
            reports.append({"case_id":case["case_id"],"label":case.get("label"),"checks":checks,"semantic_rules":[x.get("rule_id") for x in sem],"container_ids":ledger.get("existing_id_activation_refs",{}).get("container_ids",[]),"rubric_ids":ledger.get("existing_id_activation_refs",{}).get("rubric_ids",[]),"counts":counts,"live_intents":[{"intent_candidate_id":x.get("intent_candidate_id"),"machine_label":x.get("machine_label"),"composition_mode":x.get("composition_mode"),"intent_scope_candidates":x.get("intent_scope_candidates",[]),"semantic_rule_refs":x.get("semantic_rule_refs",[]),"novelty_status":x.get("novelty_status")} for x in intents]})
